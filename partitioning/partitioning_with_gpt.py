...
try:
    from graph_partitioning import partition, map_qaoa_outputs_to_full_vector, get_sub_problem
    PARTITIONING_AVAILABLE = True
except ImportError:
    PARTITIONING_AVAILABLE = False
    print("Warning: graph_partitioning module not available. Using local implementation.")
...
class PortfolioOptimizerWithGPT:
    ...
    def _load_graph(self):
        ...
        self.mu = np.array([self.graph.nodes[ticker]['annual_return'] 
                           for ticker in self.tickers])
        self.volatility = np.array([self.graph.nodes[ticker]['annual_volatility'] 
                                   for ticker in self.tickers])
    ...
    def partition_problem(self, n_partitions: Optional[int] = None, 
        ...
        if PARTITIONING_AVAILABLE and n_partitions is None:
            ...
            partitioned_sigmas = [self.sigma[np.ix_(mapping, mapping)] for mapping in mappings]
        
        # Apply max partition size constraint if specified
        if max_partition_size is not None:
            mappings, partitioned_mus, partitioned_sigmas = self._enforce_max_partition_size(
                mappings, partitioned_mus, partitioned_sigmas, max_partition_size
            )
        
        return partitioned_mus, partitioned_sigmas, mappings
    
    def _enforce_max_partition_size(self, mappings, partitioned_mus, partitioned_sigmas, 
                                   max_size):
    ...
    def portfolio_objective(self, x: np.ndarray, sigma: np.ndarray, 
                          mu: np.ndarray, q: float) -> float:
        """Calculate portfolio objective: q * risk - return."""
        return q * x.T @ sigma @ x - mu.T @ x
    
    def brute_force_optimization(self, sigma: np.ndarray, mu: np.ndarray, 
                               k: int, q: float) -> Tuple[np.ndarray, float, int]:
    ...
    def greedy_portfolio_selection(self, mu: np.ndarray, sigma: np.ndarray, 
                                  K: int, q: float) -> np.ndarray:
    ...
    def simulated_annealing(self, mu: np.ndarray, sigma: np.ndarray, 
                           K: int, q: float, max_iter: int = 2000) -> np.ndarray:
    ...
    def get_classical_baseline(self, mu: np.ndarray, sigma: np.ndarray, 
                             K: int, q: float) -> Tuple[np.ndarray, float]:
    ...
    def create_graph_for_gpt(self, sub_mu: np.ndarray, sub_sigma: np.ndarray, 
                           K: int, q: float, model_id: str = "50m_old") -> nx.Graph:
    ...
    def evaluate_gpt_circuit(self, qc, sub_problem: dict) -> Tuple[float, str]:
        ...
        try:
            ...
            print(f"    Warning: Circuit evaluation failed: {str(e)}, using greedy fallback")
        ...
        if n <= 20 and K > 0 and K <= n:
            try:
                optimal_x, optimal_objective, _ = self.brute_force_optimization(sigma, mu, K, q)
                ...
                if worst_objective != optimal_objective:
                    approx_ratio = (worst_objective - objective) / (worst_objective - optimal_objective)
                else:
                    approx_ratio = 1.0
            except:
    ...
    def solve_with_gpt(self, partitioned_mus, partitioned_sigmas, mappings,
                      K_per_partition: List[int], q: float, 
                      model_id: str = "50m_old", device: str = "cpu",
                      cached: bool = True) -> Tuple[List[int], float, List[float], Dict]:
        ...
        for i, (sub_mu, sub_sigma, k_sub, mapping) in enumerate(
            ...
            sub_problem = get_sub_problem(
                ...
                seed=1,
                pre=False
            ) if PARTITIONING_AVAILABLE else {
                'N': len(sub_mu),
                'K': k_sub,
                'q': q,
                'mu': sub_mu.tolist(),
        ...
        additional_stats = {
            ...
            'avg_circuit_tokens': np.mean([len(tokens) for tokens in generated_circuits]) if generated_circuits else 0,
    ...
    def solve_with_classical(self, partitioned_mus, partitioned_sigmas, mappings,
                           K_per_partition: List[int], q: float) -> Tuple[List[int], float]:
        ...
        for i, (sub_mu, sub_sigma, mapping, k_sub) in enumerate(
            ...
            x_sub = self.simulated_annealing(sub_mu, sub_sigma, k_sub, q * 0.8, max_iter=1000)
    ...
    def run_optimization(self, n_partitions: Optional[int] = None,
                        K_total: int = 40, q: float = 0.5,
                        max_partition_size: Optional[int] = None,
                        solver: str = "classical",
                        gpt_model_id: str = "50m_old",
                        gpt_device: str = "cpu",
                        gpt_cached: bool = True) -> OptimizationResults:
        """
        ...
        for i, mapping in enumerate(mappings):
            ...
            k_i = int(np.round(K_total * partition_size_actual / total_partition_assets))
        ...
        if current_total < K_total:
            ...
            for i in range(len(mappings)):
                if K_per_partition[i] < len(mappings[i]):
                    additional = min(K_total - current_total, 
                                   len(mappings[i]) - K_per_partition[i])
        ...
        partitioned_mus = [partitioned_mus[i] for i, m in enumerate(mappings) if len(m) > 0]
        partitioned_sigmas = [partitioned_sigmas[i] for i, m in enumerate(mappings) if len(m) > 0]
        ...
        global_x, global_obj = self.get_classical_baseline(self.mu, self.sigma, K_total, q)
        ...
        if solver == "gpt":
            pipe_idx, pipe_obj, approx_ratios, additional_stats = self.solve_with_gpt(
                partitioned_mus, partitioned_sigmas, mappings,
                K_per_partition, q, gpt_model_id, gpt_device, gpt_cached
            )
        else:
    ...
    def _print_results_summary(self, results: OptimizationResults):
        ...
        print(f"  1. Global heuristic (full problem):     {results.global_objective:.6f}")
        
        if results.additional_stats and 'classical_partitioned_objective' in results.additional_stats:
            ...
            classical_vs_global = ((classical_obj - results.global_objective) / 
                                 abs(results.global_objective) * 100)
            print(f"     → vs global: {classical_vs_global:+.1f}%")
        
        print(f"  3. {results.solver_type.upper()} solver (partitioned):    {results.pipeline_objective:.6f}")
        
        # Calculate relative differences for main solver
        pipeline_vs_global = ((results.pipeline_objective - results.global_objective) / 
                            abs(results.global_objective) * 100)
        print(f"     → vs global: {pipeline_vs_global:+.1f}%")
        
        if results.additional_stats and 'classical_partitioned_objective' in results.additional_stats:
            classical_obj = results.additional_stats['classical_partitioned_objective']
            pipeline_vs_classical = ((results.pipeline_objective - classical_obj) / 
                                   abs(classical_obj) * 100)
        ...
        if results.additional_stats and 'classical_partitioned_time' in results.additional_stats:
            print(f"  Classical partitioned: {results.additional_stats['classical_partitioned_time']:.2f}s")
        ...
        if results.subproblem_approximation_ratios:
            ...
            print(f"  Average approximation ratio: {np.mean(results.subproblem_approximation_ratios):.3f}")
            print(f"  Min/Max AR: {np.min(results.subproblem_approximation_ratios):.3f} / {np.max(results.subproblem_approximation_ratios):.3f}")
            
            # Distribution
            ratios = results.subproblem_approximation_ratios
            print(f"  Distribution:")
            print(f"    Excellent (AR > 0.9): {sum(1 for r in ratios if r > 0.9)} partitions")
            print(f"    Good (0.7 < AR ≤ 0.9): {sum(1 for r in ratios if 0.7 < r <= 0.9)} partitions")
            print(f"    Fair (0.5 < AR ≤ 0.7): {sum(1 for r in ratios if 0.5 < r <= 0.7)} partitions")
            print(f"    Poor (AR ≤ 0.5): {sum(1 for r in ratios if r <= 0.5)} partitions")
        
        print(f"\n🏆 GLOBAL PORTFOLIO ({len(results.global_solution)} assets):")
        print(", ".join(results.global_tickers[:10]) + 
              ("..." if len(results.global_tickers) > 10 else ""))
        
        print(f"\n🔗 {results.solver_type.upper()} PORTFOLIO ({len(results.pipeline_solution)} assets):")
        print(", ".join(results.pipeline_tickers[:10]) + 
    ...
    def save_results(self, results: OptimizationResults, prefix: str = ""):
        ...
        if results.additional_stats and 'classical_partitioned_objective' in results.additional_stats:
            summary['classical_partitioned_objective'] = results.additional_stats['classical_partitioned_objective']
            summary['classical_partitioned_time'] = results.additional_stats['classical_partitioned_time']
            
            # Calculate all relative differences
            summary['classical_vs_global_pct'] = ((results.additional_stats['classical_partitioned_objective'] - 
                                                  results.global_objective) / abs(results.global_objective) * 100)
            summary['pipeline_vs_global_pct'] = ((results.pipeline_objective - 
                                                results.global_objective) / abs(results.global_objective) * 100)
            summary['pipeline_vs_classical_pct'] = ((results.pipeline_objective - 
                                                    results.additional_stats['classical_partitioned_objective']) / 
                                                   abs(results.additional_stats['classical_partitioned_objective']) * 100)
        
        with open(os.path.join(self.output_dir, f"{prefix}summary_{timestamp}.json"), 'w') as f:
        ...
        global_df.to_csv(os.path.join(self.output_dir, f"{prefix}global_portfolio_{timestamp}.csv"), 
                        index=False)
        ...
        pipeline_df.to_csv(os.path.join(self.output_dir, f"{prefix}pipeline_portfolio_{timestamp}.csv"), 
                          index=False)
        
        # Save classical partitioned portfolio if available
        if results.additional_stats and 'classical_partitioned_solution' in results.additional_stats:
            classical_idx = results.additional_stats['classical_partitioned_solution']
            classical_tickers = [self.tickers[i] for i in classical_idx if i < self.n_assets]
            classical_df = pd.DataFrame({
                "ticker": classical_tickers,
                "index": classical_idx[:len(classical_tickers)]
            })
            classical_df.to_csv(os.path.join(self.output_dir, f"{prefix}classical_partitioned_{timestamp}.csv"), 
                              index=False)
...
def main(graph_path: str, 
         ...
         results_prefix: str = "") -> OptimizationResults:
    """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Portfolio Optimization with Graph Partitioning and GPT")
    ...
    parser.add_argument("--n-partitions", type=int, help="Number of partitions (if not set, uses natural clustering)")
    parser.add_argument("--max-partition-size", type=int, help="Maximum partition size")
    parser.add_argument("--k-total", type=int, default=40, help="Total assets to select")
    parser.add_argument("--risk-aversion", type=float, default=0.5, help="Risk aversion parameter")
    parser.add_argument("--solver", choices=["classical", "gpt"], default="classical", help="Solver type")
    ...
    parser.add_argument("--no-gpt-cache", action="store_true", help="Disable GPT model caching")
